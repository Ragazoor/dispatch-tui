...
fn pr_review_agent_uses_default_permission_mode() {
    ...
    let task = make_task(&repo_path);
    pr_review_agent(&task, &mock, None, None, None).unwrap();
...
fn dependabot_review_agent_uses_dependabot_prompt() {
    ...
    dependabot_review_agent(&task, &mock, None, None, None).expect("dispatch should succeed");
...
fn fix_task_agent_uses_default_permission_mode() {
    ...
    let task = make_task(&repo_path);
    fix_task_agent(&task, &mock, None, None, None).unwrap();
