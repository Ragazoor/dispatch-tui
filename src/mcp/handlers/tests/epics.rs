...
// ---------------------------------------------------------------------------
// project_id inheritance for create_epic
// ---------------------------------------------------------------------------

#[tokio::test]
async fn create_epic_inherits_project_from_caller_task() {
    let state = test_state().await;
    // Create a second project
    let project2 = state.db.create_project("Project Two", 10).await.unwrap();
    ...
    let task_id = state
        ...
        .await
        .unwrap();
    // Create an epic as CallerIdentity::Task(task_id), no explicit project_id
    let resp = call_as(
        &state,
        "tools/call",
        Some(json!({
            "name": "create_epic",
            "arguments": { "title": "Inherited Epic", "repo_path": "/repo" }
        })),
    ...
    let epic = epics
        .iter()
        .find(|e| e.title == "Inherited Epic")
        .expect("epic should be created");
    assert_eq!(
        epic.project_id, project2.id,
        "epic should inherit caller task's project_id"
    );
}

#[tokio::test]
async fn create_epic_with_parent_inherits_parent_project_over_caller_project() {
    let state = test_state().await;
    // Create two projects
    let project2 = state.db.create_project("Project Two", 10).await.unwrap();
    let project3 = state.db.create_project("Project Three", 20).await.unwrap();
    // Create a parent epic in project3
    let parent = state
        .db
        .create_epic("Parent Epic", "", "/repo", None, project3.id)
        .await
        .unwrap();
    // Create a task in project2 (different from parent's project)
    let task_id = state
        .db
        .create_task(CreateTaskRequest {
            title: "Caller Task",
        ...
        .await
        .unwrap();
    // Create a sub-epic with parent_epic_id, no explicit project_id
    let resp = call_as(
        &state,
        "tools/call",
        Some(json!({
    ...
    assert_eq!(
        sub.project_id, project3.id,
        "sub epic should inherit parent epic's project_id, not caller task's"
    );
}
