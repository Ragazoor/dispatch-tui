...
- **Two full-board refresh paths**: `exec_refresh_from_db` (command queue, inline on the render thread) carries a revision guard (`BoardReads::revision`) because it fires speculatively every 5 ticks; `reload_board` (detached, via the `spawn_refresh_*` helpers and the shared-store row pump) has none because it is only reached after something already established that the board moved. The guard reflects *why* the refresh was requested, not the reads — see the doc comments on both before unifying them. **Both paths read the same set of tables**: there used to be a narrower one covering only tasks and epics, justified by "a local write cannot have moved a repo path", which a shared store makes false.
...
**A handler that shells out must not await it here.** The paragraph above is about DB work, and a subprocess is the worse case: `SLOW_DB_CALL_THRESHOLD` does not watch it, so nothing warns, and `SUBPROCESS_TIMEOUT` is 120s — two minutes of no frames and no key handling if the command blocks on an index lock, an NFS mount or an unreachable remote. `tokio::task::spawn_blocking` alone does **not** fix this: it keeps the work off a tokio worker, but `.await`ing the handle still parks the drain loop. Send the result back as a `Message` and return the `JoinHandle` un-awaited, as `exec_check_window` and `exec_detect_default_branch` do (`src/runtime/tasks.rs`). Where the handler backs an interaction guarantee about what the user may do *while* it runs, awaiting also makes that guarantee untestable — the user cannot type during an await, so the race it describes never happens and its guard is dead code that tests pass against.
