...
| `src/tui/mod.rs` | `App` struct, lifecycle, `update()` entry point, timing constants (`STATUS_MESSAGE_TTL`, `PR_POLL_INTERVAL`, `GG_CHORD_TIMEOUT`). Column-listing helpers: `column_items_for_status_with_stats` (production render path — requires pre-computed `EpicStatsMap`, used by kanban columns); `column_items_for_visual_column` (filters by `VisualColumn` granularity, no stats needed — **no production caller**, exercised only by tests; it emits no section headers, so nothing it builds can be folded). `column_items_for_status` is test-only. |
...
| `src/models/columns.rs` | `VisualColumn` kanban board layout; `ColumnSection` — the identity of one sub-status section, and the single table every section label and sort slot is read from |
