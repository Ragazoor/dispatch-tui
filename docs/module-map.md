...
| `src/tui/mod.rs` | `App` struct, lifecycle, `update()` entry point, timing constants (`STATUS_MESSAGE_TTL`, `PR_POLL_INTERVAL`, `GG_CHORD_TIMEOUT`). Column-listing helpers: `column_items_for_status_with_stats` (the board's only column builder — requires pre-computed `EpicStatsMap`, used by kanban columns). A card's column is its `TaskStatus` and nothing else; sub-status groups cards into sections *within* a column. `column_items_for_status` is test-only. |
