...
**`spacetime/module/` is not in the workspace, so `cargo test` and `cargo clippy --all-targets` still stop at the root package.** It is a separate crate targeting wasm32, built with `spacetime build -p spacetime/module`, and its own workspace root. It is no longer unchecked: `./scripts/check-spacetime-module.sh` builds it for wasm and runs its unit tests, and both the pre-push hook and CI run that script (needs `sudo dnf install rust-std-static-wasm32-unknown-unknown`). Two root-level test files watch it from the outside — `src/spacetime/tests/module_schema.rs` compares its columns to SQLite positionally, and `tests/spacetime_module.rs` publishes it into a throwaway instance when `spacetime` is on `PATH`. See `spacetime/module/README.md`.
