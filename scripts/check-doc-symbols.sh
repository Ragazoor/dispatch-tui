...
if [[ $# -gt 0 ]]; then
    ...
    shopt -u nullglob
    # `src/spacetime/bindings/` is excluded: it is GENERATED by
    # scripts/regenerate-spacetime-bindings.sh, and its doc comments link to
    # items in the spacetimedb-sdk crate rather than to anything in this repo.
    # Every one of those reads as a phantom symbol here, and none of them is
    # editable — the files say so in their own headers.
    mapfile -t -O "${#TARGETS[@]}" TARGETS < <(
        find src -name '*.rs' -not -path 'src/spacetime/bindings/*' 2>/dev/null
    )
fi
